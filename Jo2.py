...
# 𝐃𝐞𝐯: @DEADLINEHERE5
# FINAL FIXED VERSION - No missing fragments, Full Shopify Checker API
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            try:
                ...
                if not submit_data:
                    # Shopify sometimes returns response in errors only
                    errors = resp_json.get('errors', [])
                    if errors:
                        for error in errors:
                            code = error.get('code')
                ...
                # All possible __typename values
