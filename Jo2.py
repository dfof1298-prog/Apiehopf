# Jo2_fixed.py - Shopify API Checker (FastAPI Version)
# Fixed: Invalid JSON response, Connection Pool, Rate Limiting, Memory Leaks
...
app = FastAPI(
    ...
    description="Advanced Shopify Card Checker API",
    version="2.0.0",
    lifespan=lifespan
)
...
async def make_graphql_request(session, graphql_url, params, headers, json_data, max_retries=2):
...
async def shopify_checker(
    site: str = Query(..., description="Shopify site URL (e.g., example.com or https://example.com)"),
    cc: str = Query(..., description="Card in format: CC|MM|YYYY|CVV"),
    proxy: str = Query(None, description="Proxy URL (optional) - Format: ip:port or ip:port:user:pass"),
    variant: str = Query(None, description="Product variant ID (optional)")
):
    """
    Shopify Card Checker Endpoint
    
    Checks if a credit card can be used to place an order on a Shopify site.
    
    Returns:
    - Gateway: Payment gateway used
    - Price: Total order price
    - Response: Status message (ORDER_PLACED, CARD_DECLINED, OTP_REQUIRED, etc.)
    - Status: True if successful (card charged or approved), False otherwise
    """
...
if __name__ == "__main__":
    print("=" * 50)
    print("Shopify Checker API - Fixed Version 2.0.0")
    print("=" * 50)
