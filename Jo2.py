...
# 𝐃𝐞𝐯: @DEADLINEHERE5
# FIXED: Empty submit response, Out of Memory, JSON issues
...
# Allow nested event loops (fix for Flask + asyncio)
nest_asyncio.apply()

# ====================== GRAPHQL QUERIES (SAME AS YOUR ORIGINAL) ======================
# [تم الاحتفاظ بكل الـ QUERY_PROPOSAL_SHIPPING, QUERY_PROPOSAL_DELIVERY, MUTATION_SUBMIT, QUERY_POLL كما هي]
# لأنها طويلة جداً، أضفتها في نهاية الملف عشان الكود يبقى مقروء
# ====================================================================================

# Simplified for brevity - use your original queries here
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            try:
                ...
                if not submit_data:
                    # ========== الحل النهائي لمشكلة Empty submit response ==========
                    # Shopify أحياناً بيرجع رد في errors بس
                    errors = resp_json.get('errors', [])
                    if errors:
                        for error in errors:
                            code = error.get('code')
                            ...
                            if code:
                                ...
                                return False, message[:50], gateway, total_price, currency
                    
                    # جرب تشوف إذا كان في redirect
                    if 'redirect' in text.lower() or 'redirection' in text.lower():
                        return False, "REDIRECT_REQUIRED", gateway, total_price, currency
                    
                    return False, "Empty submit response - Site may not support direct checkout", gateway, total_price, currency
                
                result_type = submit_data.get('__typename', '')
                
                # ========== إضافة جميع الأنواع الممكنة ==========
                if result_type in ['SubmitSuccess', 'SubmittedForCompletion', 'SubmitAlreadyAccepted', 'SubmitProcessing', 'SubmitPending']:
                    ...
                    if receipt:
                        ...
                        # يمكن تكون خلصت من غير receipt
                        return True, "ORDER_PLACED", gateway, total_price, currency
                
                elif result_type in ['SubmitFailed', 'SubmitThrottled']:
                    reason = submit_data.get('reason', 'Unknown reason')
                    if 'throttl' in str(reason).lower():
                        await asyncio.sleep(3)
                        # حاول مرة تانية (الـ retry هيتم في upper level)
                        return False, "THROTTLED_RETRY", gateway, total_price, currency
                ...
                # ========== معالجة أي __typename جديد ==========
                else:
                    # لو نوع جديد، حاول تستخرج الـ receipt لو موجود
                    receipt = submit_data.get('receipt')
                    if receipt:
                        ...
                        # لو مش عارف، حاول تشوف إذا في order created
                        if 'order' in str(submit_data).lower():
                            return True, "ORDER_PLACED", gateway, total_price, currency
                        return False, f"Unknown response type: {result_type}", gateway, total_price, currency
                ...
                # لو الـ JSON مش صحيح، حاول تشوف إذا كان في Order Created
                if 'order' in text.lower() or 'processed' in text.lower():
                    return True, "ORDER_PLACED", gateway, total_price, currency
    ...
    try:
        ...
        if not site:
            return jsonify({
                ...
                "Gateway": "UNKNOWN",
                "Price": 0.0,
                "Response": "ERROR: Missing site parameter"
            }), 400
        ...
        if not cc_string:
            return jsonify({
                ...
                "Gateway": "UNKNOWN",
                "Price": 0.0,
                "Response": "ERROR: Missing cc parameter"
            }), 400
        ...
        # Use asyncio.run() with proper handling
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                success, message, gateway, price, currency = future.result()
...
def not_found(e):
    return jsonify({"error": "Endpoint not found", "status": False}), 404
